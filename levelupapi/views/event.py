...
from levelupapi.models import Game, Event, Player, AttendingPlayer
...
class Events(ViewSet):
    ...
    def list(self, request):
        ...
        for event in events:
            ...
            try:
                AttendingPlayer.objects.get(event=event, player=player)
                event.joined = True
            except AttendingPlayer.DoesNotExist:
                event.joined = False
    ...
    def signup(self, request, pk=None):
        """Managing players signing up for events"""

        # A player wants to sign up for an event
        if request.method == "POST":
            ...
            try:
                ...
                registration = AttendingPlayer.objects.get(
                    event=event, player=player)
                return Response(
                    {'message': 'PLayer already signed up this event.'},
                    status=status.HTTP_422_UNPROCESSABLE_ENTITY
                )
            except AttendingPlayer.DoesNotExist:
                # The user is not signed up.
                registration = AttendingPlayer()
...
class EventSerializer(serializers.ModelSerializer):
    ...
    organizer = EventPlayerSerializer(many=False)
